...
def forgot_password(request):
	...
	if user.exists():
		...
		#print(reset_link)
		html_content = get_template('email/user/reset_password.html').render({'reset_link':reset_link})
	...
	if request.data.get('id') and request.data.get('model') and request.data.get('isactivestatus'):
		...
		if isactivestatus in listOfStatus:
			...
			if email_content.exists():
				...
				context = {
					'user': FullName, 
					'asset':mark_safe('<a href="{}">Asset</a>'.format(url)),
				}
