...
class ContactViewSet(CommonViewset, generics.RetrieveAPIView):
	...
	def aircraft_by_user(self, request, *args, **kwargs):
		...
		print(length)
		user_aircraft = AbAircrafts.objects.filter(user=contact.user)[0:int(length)]
...
def send_message(request):
	...
	if model in allowed_models:
		...
		ableads = AbLeads(is_active=1, leadable_type='App\\{}'.format(model), leadable_id=model_id, creator=request.user)
		if model != 'Company':
		...
		if model != 'Company':
			...
			context = {'email_content': mark_safe(message_content)}

			subject = 'New message | Airbook'
			...
			subject = 'Copy - New message | Airbook'
