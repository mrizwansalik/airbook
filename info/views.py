	...
	for model in models:
		...
		if model_ in allowed_models:
			...
			if model == 'AbContacts' and isRoleUser(request.user):
				queryset = abmodel.objects.filter(deleted_at=None).filter(user__groups__name__in=['user','User']).filter(user=request.user)
...
def filter_base_related_abmodels(request):
	...
	if related_name in ['contact','abwanteds']:
		...
		kwargs = {"deleted_at":None, "isactivestatus":"Approved"}
