...
import { showFilters, formatDate, next_link, previous_link, likes, getData, ucwords, availFor, getMediaUrl, randomizeOrdering, getTitle } from '../../../helpers/functions';
              ...
              <div className="modal-block">
                <div className="modal-form-block w-form">
                  <form id="wf-form-message-modal" action="/" name="wf-form-message-modal" data-name="message modal" className="asset-modal-form" style={modal.form}>
                    <div className="text-block">Message, {currentIndex.primary_contact && currentIndex.primary_contact.first_name} {currentIndex.primary_contact && currentIndex.primary_contact.last_name}</div><a href="#" target="_blank" className="modal-regarding">{currentIndex.title} available for {currentIndex.offer_for}</a>
            ...
            <div className="ab-page-main-content">
              ...
              <div className="list-block">
                { availFor(this) !== '' && <div className="ab-page-title-block">
                  ...
                  <p className="ab-page-description">{"availability for " + availFor(this)}</p>
                </div> }
                <div className="w-layout-grid ab-list-grid">

                  {aircrafts.length > 0 && aircrafts.map((aircraft, index) => {
                      ...
                      <div className="item-flex-header">
                        ...
                        <div className="item-like-block">
                          <div className="ab-likes w-embed" onClick={() => this.getlikes(aircraft.id, 'Aircraft')}>
                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" className={aircraft.liked !== 0 ? 'liked' : ''}>
                        ...
                        <div className="item-info-block">
                          <div className="flex-specsbox">
                            ...
                            {isAuthenticate ?
                              ...
                              <div className="item-specs reg-value">{aircraft.registration_number ? "REG " + aircraft.registration_number:''}</div>
                              </>
                              : <>
                             <div className="item-specs sn-value">{aircraft.csn ? 
                          ...
                          <div className="flex-specsbox">
                            <p className="item-specs">
                              {aircraft.type && aircraft.type.type ?
                                ...
                                ''}
                              { formatDate(aircraft.yom, 'yom') !== '' ? <span className="spec-span dot-before">
                                 {"YOM " + formatDate(aircraft.yom, 'yom') }
                              </span> :''}
                              {aircraft.tsn ? <span className="spec-span dot-before tsn">
                              {"TSN " + aircraft.tsn}</span>:''} 
                              {aircraft.status ? <span className="spec-span dot-before item-status">{aircraft.status}</span> : ''}
                            </p>
                          </div>
                          <div className="flex-specsbox">
                            {isAuthenticate && aircraft.primary_contact ?
                              ...
                              <Link href={{ pathname: `/contact/[detail]` }} as={`/contact/${aircraft.primary_contact && aircraft.primary_contact.id + '-' + aircraft.primary_contact.first_name + "-" + aircraft.primary_contact.last_name}`}>
                                <a target="_blank" className="publisher-link">
                                  {aircraft.primary_contact ? (aircraft.primary_contact.first_name + " " + aircraft.primary_contact.last_name) : 'N/A'}
                                </a>
                              </Link>
                              </> : (!isAuthenticate ? 
                              <div className="locked-item-block">
                        ...
                        <div className="ab-list-item-widget">
                          <div data-delay="0" className="ab-list-item-menu w-dropdown">
                            ...
                            <nav className={`ab-list-item-dropdown w-dropdown-list ${ids.includes(index) && 'w--open'}`} >
                              ...
                              {aircraft.favourite !== true && <a href="#" className="list-item-menu-link w-dropdown-link" onClick={()=>{this.addFavorite(aircraft.id, 'App\\Aircraft')}}>Add to favorite</a>}
                ...
                <div className="list-pagniation">
                  {previous &&
                    <Link href={`/aircraft/p/[list]?filters=${JSON.stringify(this.state.selected_filters)}&search=${search_str}`} as={`/aircraft/p/${previous}`}>
                      <a className="pagination-button w-button">Previous</a>
                    </Link>}
                  {next &&
                    <Link href={`/aircraft/p/[list]?filters=${JSON.stringify(this.state.selected_filters)}&search=${search_str}`} as={`/aircraft/p/${next}`}>
                      <a className="pagination-button w-button">Next</a>
                    </Link>}
                </div>
                {aircrafts.length === 0 && <div className="list-empty">
