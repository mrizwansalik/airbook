...
import { showFilters, next_link, previous_link, getData, likes, ucwords, getMediaUrl, randomizeOrdering, checkValue } from '../../../helpers/functions';
            ...
            <div className="ab-page-main-content">
              <div className="filter-block">
                ...
                <div className="filter-block-content">
                  <div className="filter-form-block w-form">
                    <form id="wf-form-filter-form" action="submit" name="wf-form-filter-form" data-name="filter form" className="filter-form">
              ...
              <div className="list-block">
                  ...
                  {contacts && contacts.map((contact, index) => {
                    return <div className="ab-list-item-wrapper contact" key={index}>
                      <div className="item-flex-header">
                        <div className="item-title-block">
                          ...
                          <div className="item-h3-block">
                            <h3 className="subtitle">{contact.job_title && contact.job_title.name}</h3>
                          </div>
                        </div>
                      ...
                      <div className="item-data-flex">
                        ...
                        <Link href={{ pathname: `/contact/detail` }} as={`/contact/${contact.id + '-' + contact.first_name + "-" + contact.last_name}`}>
                          <a
                            className="item-image w-inline-block" style={{backgroundImage: `url(${getMediaUrl(contact.media)})`}}
                          />
                        </Link> : ''}
                        <div className="item-info-block">
                          ...
                          {contact.address  && <div className="flex-specsbox">
                           {checkValue(contact.country && contact.country.flag)?  <div className="svg-flag" style={{backgroundImage:`url(/static/media/site/flags/${contact.country.flag})`}}/>: ''}
                            <div className="country-name">{contact.country.name}</div>
                          </div>}
                ...
                <div className="list-pagniation">
                    {previous &&
                    <Link href={`/contact/p/[list]?filters=${JSON.stringify(this.state.selected_filters)}&search=${search_str}`} as={`/contact/p/${previous}`}>
                      <a className="pagination-button w-button">Previous</a>
                    </Link> }
                    {next && 
                    <Link href={`/contact/p/[list]?filters=${JSON.stringify(this.state.selected_filters)}&search=${search_str}`} as={`/contact/p/${next}`}>
                      <a className="pagination-button w-button">Next</a>
                    </Link>}
                </div>
                {contacts.length === 0 && <div className="list-empty">
