...
import { formatDate, checkValue, getData, getMediaUrl } from '../../helpers/functions';
...
class ContactDetailPage extends React.Component {
  ...
  render() {
    let { contact, submitting, isAuthenticate, related, sideAdvert, topAdvert, relatedAircrafts } = this.state;
    return (
      <>
        <Header />
        <div className="ab-page-content">
          <div className="ab-container w-container">
            ...
            {topAdvert && topAdvert.media != null ? <a href="#" target="_blank" className="ab-top-page-advert w-inline-block" style={{ backgroundImage: 'url(/static/images/' + topAdvert.media.original_file_name + ')' }}></a>
              : ""}
            <h1 className="details-page-headline">{checkValue(contact.first_name) + " " + checkValue(contact.last_name)}</h1>
            <div className="publish-date-block">
              ...
              <div className="date-created-updated">
                {"Published " + formatDate(contact.created_at) + " - Updated " + formatDate(contact.updated_at)}
              </div>
            </div>
            <div className="breadcrumb-block">
              ...
              <div className="current-asset">{checkValue(contact.first_name) + " " + checkValue(contact.last_name)}</div>
            </div>
              ...
              <div className="flex-col-left">
                <div className="details-data-block">
                  <div className="label-block">
                    ...
                    <h3 className="details-block-headline">Profile Details</h3>
                  </div>
                  ...
                  {checkValue(contact.job_title && contact.job_title.name)?
                  <div className="data-block">
                    <div className="data-label">Job Title</div>
                    <p className="data-value">{contact.job_title && contact.job_title.name}</p>
                  </div>:''
                  }
                  {checkValue(contact.country && contact.country.name)?
                  <div className="data-block">
                    <div className="data-label">Company</div>
                    {checkValue(contact.company && contact.company.name) !== 'N/A' ?<Link href={{ pathname: `/company/detail` }} as={`/company/${contact && contact.company && (contact.company.id + "-" + contact.company.name)}`}>
                  ...
                  <div className="data-block">
                    ...
                    <p className="data-value">{contact.country && contact.country.name}</p>
                  </div>:''
                  }
                  <div className="data-block">
                    <div className="data-label">Member Since</div>
                    <p className="data-value">{formatDate(contact.created_at, 'availability')}</p>
                  </div>
                </div>
                <div className="details-data-block">
                ...
                <div className="details-data-block">
                  <div className="label-block">
                    ...
                    {checkValue(contact.first_name)? 
                    <h3 className="details-block-headline">On Airbook by {contact.first_name}</h3>:''}
                  </div>
                  {relatedAircrafts.length > 0 && relatedAircrafts.map((aircraft, index) => {
                    return <div className="data-block file-attachment" key={index}>
                      <Link href={{ pathname: `/aircraft/[detail]` }} as={`/aircraft/${aircraft.id + "-" + aircraft.title}`}>
                ...
                <div className="related-items-block">
                        ...
                        <div className="ab-list-item-wrapper contact">
                          <div className="item-flex-header">
                            <div className="item-title-block">
                              ...
                              <Link href={{ pathname: `/contact/[detail]` }} as={`/contact/${related.id + '-' + related.first_name + "-" + related.last_name}`}>
                                <a className="item-link-block w-inline-block">
                                  <h2 className="item-composite-title">{related.first_name + " " + related.last_name}</h2>
                                </a>
                              </Link>
                              <div className="item-h3-block">
                                <h3 className="subtitle">{related.job_title && related.job_title.name}</h3>
                              </div>
                            </div>
                          </div>
                          ...
                          {getMediaUrl(contact.media)? 
                            ...
                            <div className="item-info-block">
                              <div className="flex-specsbox">
                                {related.connected &&  related.connected !== 0 ? (
                                    <div className = "connection-status"> In connections </div>
                                  ):(
                                    <a href="#" className="contact-connect w-button" onClick={()=>{this.In_connection(related)}}>
                                    Connect
                                    </a>
                                 )
                                }
                              ...
                              <div className="flex-specsbox">
                               {checkValue(contact.country && contact.country.flag)?  <div className="svg-flag" style={{backgroundImage:`url(/static/media/site/flags/${contact.country.flag})`}}/>:''}
                                <div className="country-name">{related.address ? related.address : 'Address not Provided'}</div>
                              </div>:''
                              }
                              {checkValue(related.company && related.company.name)?
                              <div className="flex-specsbox">
                                ...
                                {related.company ? <Link href={{ pathname: `/company/detail` }} as={`/company/${related && related.company && (related.company.id + "-" + related.company.name)}`}>
                  ...
                  <div className="list-empty mb-10">
                    ...
                    <div className="no-results-message">No related contact.</div>
                  </div>}
                </div>
              </div>
              ...
              {contact.media && contact.media.exist ?
                <div className="image-gallery">
                  <a href="#" className="main-lightbox w-inline-block w-lightbox">
                    <style jsx>
                      {
                        `.profile-image{
														background-image: url(${process.env.NEXT_PUBLIC_API_URL}static/media/uploads/${contact.media ? contact.media.original_file_name : "placeholder.png"})};
												}`
                      }
                    </style>
                ...
                <div className="sidebar-data-wrapper">
                  <div className="label-block">
                    ...
                    <h3 className="details-block-headline">Contact Preferences</h3>
                  </div>
                  ...
                  <div className="sidebar-data-block">
                    {contact.connected &&  contact.connected !== 0 ? (
                        ...
                        <a href="#" className="contact-connect sidebar-connect w-button" onClick={()=>{this.In_connection(contact)}}>
                        Connect
                        </a>
                      )
                    }
                ...
                <div className="sidebar-data-wrapper">
                  <div className="label-block">
                    ...
                    <h3 className="details-block-headline">{"Message " + checkValue(contact.first_name)}</h3>:<h3 className="details-block-headline">Message To Publisher</h3> }
                  </div>
                ...
                <a onClick={()=>this.updateProfile()} className="profile-update-block w-inline-block" href="#">
                ...
                <div className="sidebar-data-wrapper">
                  ...
                  <div className="share-flex">
                    <a href={`https://www.linkedin.com/shareArticle?mini=true&url=${process.env.NEXT_PUBLIC_API_URL}contact/${this.props.params.detail}`} target="_blank" className="share-widget w-inline-block">
                    ...
                    <a href={`https://www.facebook.com/sharer/sharer.php?u=${process.env.NEXT_PUBLIC_API_URL}contact/${this.props.params.detail}`} target="_blank" className="share-widget w-inline-block">
                      <div className="w-embed">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width={24}
                    ...
                    <a href={`https://api.whatsapp.com/send?phone=whatsappphonenumber&text=${process.env.NEXT_PUBLIC_API_URL}contact/${this.props.params.detail}`} target="_blank" className="share-widget w-inline-block">
                      <div className="w-embed">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width={24}
