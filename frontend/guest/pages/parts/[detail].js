...
import { formatDate, checkValue, getData } from '../../helpers/functions';
        ...
        <div className="ab-page-content">
          <div className="ab-container w-container">
            ...
            {topAdvert && topAdvert.media != null ? <a href="#" target="_blank" className="ab-top-page-advert w-inline-block" style={{ backgroundImage: 'url(/static/images/' + topAdvert.media.original_file_name + ')' }}></a>
              : ""}
            <h1 className="details-page-headline">{`PN ${checkValue(part.part_number)}`}</h1>
            <div className="publish-date-block">
              ...
              <div className="date-created-updated">
                {"Published " + formatDate(part.created_at) + " - Updated " + formatDate(part.updated_at)}
              </div>
            </div>
            <div className="breadcrumb-block">
              ...
              <div className="current-asset">{`PN ${checkValue(part.part_number)}`}</div>
            </div>
              ...
              <div className="flex-col-left">
                <div className="details-data-block">
                  ...
                  <div className="data-block">
                    ...
                    <p className="data-value">{part.alternate_part_number}</p>
                  </div>: ''}
                  ...
                  <div className="data-block">
                    ...
                    <p className="data-value">{part.condition && part.condition.name}</p>
                  </div>:''}
                  ...
                  <div className="data-block">
                    ...
                    <p className="data-value">{part.release && part.release.name}</p>
                  </div>:''}
                  {checkValue(part.location && part.location.name)? 
                  <div className="data-block">
                    <div className="data-label">Location</div>
                    {checkValue(part.location && part.location.flag) ? <div className="svg-flag" style={{backgroundImage:`url(/static/media/site/flags/${part.location.flag})`}}/> : ''}
                    <p className="data-value">{part.location && part.location.name}</p>
                  </div>: ''}
                </div>
                <div className="details-data-block">
                  {checkValue(part.price || part.owner && part.owner.name || part.seller && part.seller.name || part.unit_measure)? 
                  <div className="label-block">
                    <div className="ab-svg-icon details-page w-embed">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
              ...
              <div className="flex-col-right">
                <div className="sidebar-data-wrapper">
                  <div className="label-block">
                    ...
                    <h3 className="details-block-headline">Send RFQ</h3>:<h3 className="details-block-headline">Message To Publisher</h3> }
                  </div>
                  ...
                  <div className="sidebar-publisher">
                    ...
                    <div className="sidebar-publisher-info">
                      <div className="profile-block">
                        <Link href={{ pathname: `/contact/[detail]` }} as={`/contact/${part.primary_contact && (part.primary_contact.id + '-' + part.primary_contact.first_name + "-" + part.primary_contact.last_name)}`}>
                          <a target="_blank" className="sidebar-link">
                            {part.primary_contact && part.primary_contact.first_name} {part.primary_contact && part.primary_contact.last_name}
                          </a>
                        </Link>
                        <div className="sidebar-publisher-txt">{part.primary_contact && part.primary_contact.job_title && part.primary_contact.job_title.name}</div>
                        <div className="member-info">Member since {formatDate(part.primary_contact && part.primary_contact.created_at)}</div>
                        <Link href={{ pathname: `/company/detail` }} as={`/company/${part.primary_contact && part.primary_contact.company && (part.primary_contact.company.id && part.primary_contact.company.id + "-" + part.primary_contact.company.name)}`}>
                          <a target="_blank" className="sidebar-link org-name">
                            {checkValue(part.primary_contact && part.primary_contact.company && part.primary_contact.company.name)}
                          </a>
                        </Link>
                      </div>
                      {checkValue(part.primary_contact && part.primary_contact.country && part.primary_contact.country.name)? 
                      <div className="sidebar-country">
                       {checkValue(part.primary_contact.country && part.primary_contact.country.flag)? <div className="svg-flag" style={{backgroundImage:`url(/static/media/site/flags/${part.primary_contact.country.flag})`}}/> : "" }
                        <div className="ab-list-data-txt">{part.primary_contact && part.primary_contact.country && part.primary_contact.country.name}</div>
                      </div>: ''}
                    </div>
                  </div>:
                ...
                <div className="sidebar-data-wrapper">
                  ...
                  <div className="share-flex">
                    <a href={`https://www.linkedin.com/shareArticle?mini=true&url=${process.env.NEXT_PUBLIC_API_URL}parts/${this.props.params.detail}`} target="_blank" className="share-widget w-inline-block">
                    ...
                    <a href={`https://www.facebook.com/sharer/sharer.php?u=${process.env.NEXT_PUBLIC_API_URL}parts/${this.props.params.detail}`} target="_blank" className="share-widget w-inline-block">
                      <div className="w-embed">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width={24}
                    ...
                    <a href={`https://api.whatsapp.com/send?phone=whatsappphonenumber&text=${process.env.NEXT_PUBLIC_API_URL}parts/${this.props.params.detail}`} target="_blank" className="share-widget w-inline-block">
                      <div className="w-embed">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width={24}
