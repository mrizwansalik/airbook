...
import { formatDate, filteredValues, selectFilter, next_link, previous_link, getData, randomizeOrdering, checkValue } from '../../../helpers/functions';
...
class PartsListPage extends React.Component {
  ...
  async loadModels() {
    let { conditions, countries } = this.state;
    let models = {
      'AbConditions': { },
      'AbCountries': {  },
    }
    await post('abmodels', { models: models }).then(function (response) {
      ...
      conditions.unshift({label:'ALL', name:'ALL'})
    })
  ...
  filterValues (value, key){
    let {conditions, selected_filters, all_filter_selected} = this.state
     value ='all'
     if(all_filter_selected == true){
      selected_filters[key]= []
      all_filter_selected= false
     }else{
       this.setState({all_filter_selected: true})
     }
  }
  getListings(url) {
  ...
  render() {
    let { search_str, modal, part, parts, all_selected, part_requests, RFQMessage, previous, next, conditions, countries, isAuthenticate, topAdvert } = this.state;
              ...
              <div className="modal-form-block w-form">
                <form id="wf-form-message-modal" action="/" name="wf-form-message-modal" data-name="message modal" className="asset-modal-form" style={modal.form}>
                  ...
                  <div style={this.style.RFQList}>
                    {part_requests.map((pr, i) => {
                      if(part === null || pr.id === part.id) {
                        return <div className="pn-qty-block">
                          <div className="parts-list-modal-qty">
                          <a href="#" className="modal-regarding">PN {pr.part_number}</a> 
                          <span className="modal-pn-cond">NE</span> 
                          </div>
                        <input type="number" value={pr.qty ? pr.qty:''} onChange={(e)=>this.setRFQuantity(e, i)} className="modal-qty w-input" maxLength="256" name="Quantity" data-name="Quantity" placeholder="Qty" id="Quantity-6" required=""/>
                      </div>
                    }
                    })}
                    </div>
                    <textarea data-name="Message 2" onChange={(e)=>this.setState({RFQMessage:e.target.value})} maxLength="5000" id="Message-2" name="Message-2" required="" placeholder="request details" className="modal-msg-box w-input">{RFQMessage}</textarea>
                  ...
                  <div className="modal-info">
                    ...
                    <div className="message-copy-alert">You will receive a copy of this message.</div>
                  </div>
                </form>
                <div className="modal-message-success w-form-done" style={modal.success}>
                  <div className="text-block-2"><span>Thank you! </span><br/>Message sent to Zulqarnain.<br/><br/>You will also receive a copy of this message.</div>
                </div>
                <div className="w-form-fail" style={modal.error}>
                  <div>Oops! Something went wrong. Please try again in a moment.</div>
                </div>
              </div>
              <div className="asset-message-modal-brand"><img src="images/Airbook.svg" width="100" alt="Airbook - Aviation Market Place" className="image"/></div>
            </div>
          </div>
        </div>
                ...
                >
                  <div className="parts-search-block searchbox">
                    <textarea
                      ...
                      defaultValue={""}
                      onChange={(e) => this.setState({search_items:e.target.value})}
                    />
                    <input
                      type="submit"
                      defaultValue="Search"
                  ...
                  <div className="parts-search-block">
                    ...
                    <div>
                      <div className="w-layout-grid pn-condition-grid">
                        {conditions && conditions.map((condition, index) => {
                          return <label className="w-checkbox pncondition" key={"condition" + index}>
                            <div className="w-checkbox-input w-checkbox-input--inputType-custom pn-condition-check " />
                            <input
                              type="checkbox"
                              id={condition.id}
                              name={condition.name}
                              ...
                              onChange={() => { filteredValues(condition.value, 'condition', this) }}
                            />
                            <span
                    ...
                    <div>
                      <select
                        ...
                        onChange={(e) => { selectFilter(e, this, 'APUs') }}
                      >
                        <option value="">All countries..</option>
                        {countries && countries.map((country, index) => {
                          return <option key={"country" + index} value={country.id} onClick={(e) => { selectFilter(e, country.id, this) }}>{country.name}</option>
                        })}
                      </select>
                    </div>
                  </div>
            ...
            <div className="parts-list-wrapper-web">
              <div className="part-list-sticky-row">
                <div className="basket-block">
                  <div className="text-block-5">{part_requests.length} Selected</div>
                  <a
                    ...
                    <div className="checbox-form w-form">
                      ...
                      >
                        <label className="w-checkbox checkbox-block checkbox-field">
                          <div className={"w-checkbox-input w-checkbox-input--inputType-custom part-select-check-web checkall" + ( part_requests.length === parts.length ? " w--redirected-checked":'')} />
                          <input
                            type="checkbox"
                            id="checkbox-all"
                            name="checkbox-all"
                            checked={part_requests.length === parts.length ? true:false}
              ...
              <div className="part-list-part-row">
                {parts && parts.map((part, index) => {
                  return <div key={"part" + index}><div className="part-list-flex-data-row">
                    <div className="part-list-col col-check">
                      <div className="checbox-form w-form">
                        <form
                          id="email-form-2"
                        ...
                        >
                          <label className="w-checkbox checkbox-block checkbox-field">
                            <div className={"w-checkbox-input w-checkbox-input--inputType-custom part-select-check-web " + (part_requests.filter(pr => pr.id === part.id).length ? " w--redirected-checked":'')} />
                            <input
                              type="checkbox"
                              id={"checkbox-"+index}
                              name={"checkbox-"+index}
                              data-name={"checkbox-"+index}
                              checked={part_requests.filter(pr => pr.id === part.id).length ? true:false}
                    ...
                    <div className="part-list-col col-pn">
                      <Link href={{ pathname: `/parts/[detail]` }} as={`/parts/${(part.id + "-" + (part.condition && part.condition.name.split('/').join('-')))}`}>
                    ...
                    {checkValue(part.description)?
                      <div className="part-list-data-label">{part.description}</div>: ""}
                    </div>
                    <div className="part-list-col col-cond">
                    {checkValue(part.condition && part.condition.name) ?
                      <div className="part-list-data-label">{part.condition && part.condition.name}</div>: ""}
                    </div>
                    <div className="part-list-col col-qty">
                    {checkValue(part.quantity)? <div className="part-list-data-label">{part.quantity}</div>: ""}
                    </div>
                    <div className="part-list-col col-price">
                      {checkValue(part.price && part.price >=1)?
                      <div className="part-list-data-label">{"$" + part.price}</div>: ""}
                    </div>
                    <div className="part-list-col col-date">
                    {formatDate(part.created_at)?
                      <div className="part-list-data-label">{formatDate(part.created_at)}</div>: ""}
                    </div>
                    <div className="part-list-col col-rfq">
                      <div className="part-list-send-rfq" 
                    onClick={()=>{this.showModal(part)}}>RFQ</div>
                    </div>
                  </div>
                      ...
                      {isAuthenticate ?
                        <>
                          <div className="parts-list-org-info">
                            <Link href={{ pathname: `/company/detail` }} as={`/company/${part.primary_contact.company && part.primary_contact.company.id + "-" + part.primary_contact.company.name}`}>
                              <a target="_blank" className="part-list-org-link">{part.primary_contact && part.primary_contact.company && part.primary_contact.company.name} </a>
                            </Link>
                          </div>
                          <div className="parts-list-org-info">
                            <a href="#" target="_blank" className="part-list-org-link">{part.primary_contact && part.primary_contact.company && part.primary_contact.company.business_phone}</a>
                          </div>
                          <div className="parts-list-org-info">
                            <a href="#" target="_blank" className="part-list-org-link">{part.user && part.user.email}</a>
                          </div>
                          <div className="parts-list-org-info">
                            <a href={this.formatlink(part.primary_contact && part.primary_contact.company && part.primary_contact.company.website)} target="_blank" className="part-list-org-link">
                              {part.primary_contact && part.primary_contact.company && part.primary_contact.company.website}
                            </a>
                          </div>
                        </> :
                        <div className="locked-item-block parts-lock">
                          ...
                          <div className="locked-alert">Login to view contact</div>
                        </div>
                      }
                      <div className="text-block-4">{part.primary_contact && part.primary_contact.company && part.primary_contact.company.country && part.primary_contact.company.country.name}</div>
                    </div>
                  </div>
                })}
              </div>
              <div className="list-pagniation" data={search_str}>
                {previous &&
                  <Link href={`/parts/p/[list]?filters=${JSON.stringify(this.state.selected_filters)}&search=${search_str}`} as={`/parts/p/${previous}`}>
                    <a className="pagination-button w-button">Previous</a>
                  </Link>}
                {next &&
                  <Link href={`/parts/p/[list]?filters=${JSON.stringify(this.state.selected_filters)}&search=${search_str}`} as={`/parts/p/${next}`}>
                    <a className="pagination-button w-button">Next</a>
                  </Link>}
              </div>
              {parts.length === 0 && <div className="list-empty">
              ...
              {topAdvert && topAdvert.media != null ? <a href="#" target="_blank" className="asset-list-footer-advert w-inline-block" style={{ backgroundImage: 'url(/static/images/' + topAdvert.media.original_file_name + ')' }}>
              </a>
                :
                ""}
            </div>
