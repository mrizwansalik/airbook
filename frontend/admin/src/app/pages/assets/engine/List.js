    ...
    function loadDatatable() {
        ...
        let columnDefs = [
            ...
            },
            {
                targets: 2,
                "createdCell": function (td, cellData, rowData, row, col) {
                    const name = rowData.model__name+' '+rowData.manufacturer__name;
                    const engine_slug = rowData.id+'-'+name.replace(/\s+/g, '-').toLowerCase()
                    ReactDOM.render(
                        <a target="_blank" style={{ cursor: 'pointer' }} href={"/engine/" + engine_slug.replace(/ /g,'-').replace(/\//g,'-') + "/"}>
                            {cellData}
                        </a>
                        , td
                    );
    ...
    return (
        ...
        <div className={classes.root}>
            ...
            <Paper className={classes.paper}>
                <div className={classes.tableWrapper}>
                    <div className="kt-portlet kt-portlet--mobile">
                        <EnhancedTableToolbar exportSpinner={exportSpinner} setExpSpin={setExpSpin} setTemp={setTemp}
                                              setRowsData={setRowsData} rows={rows} setSelectedMany={setSelectedMany}
                                              setSelected={setSelected} selected={selected}
                                              numSelected={selected.length} type={type} csvData={csvData}
            ...
            <Modal show={showEmail} onHide={handleEmailModalClose} centered>
                <Modal.Header closeButton>
                    <Modal.Title>Send mail to {statusData ? statusData.isactivestatus : ''}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <textarea rows="5" onChange={(e) => setReason(e.target.value)}
                              style={{display: 'block', width: "100%"}}/>
                </Modal.Body>
                <Modal.Footer>
                    <Button variant="success"
                            className={showSpinner === true ? "kt-spinner kt-spinner--right kt-spinner--sm kt-spinner--light" : ''}
